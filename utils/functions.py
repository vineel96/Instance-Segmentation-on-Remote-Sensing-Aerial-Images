...
class DropBlock(nn.Module):
    def __init__(self, block_size=7, keep_prob=0.5):
        super().__init__()
...
def make_net(in_channels, conf, which_head, include_last_relu=True):
    """
    ...
    def make_layer(layer_cfg):
        ...
        #     return [layer]
        # else:
        if kernel_size==0:  # added, for dropblock no need of relu again
            return [layer]
        else:
            return [layer, nn.ReLU(inplace=True)]

    # Use sum to concat together all the component layer lists
    if which_head=="proto_net":
        conf.insert(4,(None, 0, {})) # add dropblock to conf after interpolation layer, some conf to do filter
